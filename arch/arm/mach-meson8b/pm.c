...
{
    if(power_on == OFF) {
        printk("ijay m8ref_set_vccx2: OFF");
		...
		//disable netled when stanby
        #elif defined CONFIG_MESON8B_512M_TH01
        ...
        printk("ijay m8ref_set_vccx2: ON");
        CLEAR_AOBUS_REG_MASK(AO_GPIO_O_EN_N, 1<<15);
        SET_AOBUS_REG_MASK(AO_GPIO_O_EN_N, 1<<31);
		printk("code run here2\n");
		#if defined CONFIG_MESON8B_512M_TH01
		CLEAR_AOBUS_REG_MASK(AO_RTI_PIN_MUX_REG, (1<<11)|(1<<25));
		CLEAR_AOBUS_REG_MASK(AO_GPIO_O_EN_N, 1<<1);
