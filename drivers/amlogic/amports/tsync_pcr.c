...
#define OPEN_RECOVERY_THRESHOLD 90000
#define CLOSE_RECOVERY_THRESHOLD 9000
...
static u32 tsync_pcr_min_cache_time = TIME_UNIT90K*0.5;			//TIME_UNIT90K*0.2;


// use for pcr invalid mode
static u32 tsync_pcr_max_delay_time = TIME_UNIT90K*3;				//TIME_UNwIT90K*3;
...
static unsigned long tsync_pcr_check(void)
{
    ...
    int64_t last_checkout_vpts = 0;
    int64_t last_checkout_apts = 0;
    ...
    if ((!(tsync_pcr_inited_flag&TSYNC_PCR_INITCHECK_VPTS)) && (!(tsync_pcr_inited_flag&TSYNC_PCR_INITCHECK_PCR)) && (!(tsync_pcr_inited_flag&TSYNC_PCR_INITCHECK_APTS))){
        ...
        if (cur_system_time - first_time_record < 270000) {
            //tsync_pcr_pcrscr_set();
			//printk("live  line 626 cur_system_time 0x%llx first_time_record 0x%llx\n",cur_system_time, first_time_record);
        }
        else {
            tsync_pcr_inited_mode=INIT_PRIORITY_VIDEO;
            tsync_pcr_pcrscr_set();
			//printk("live  line 631 cur_system_time 0x%llx first_time_record 0x%llx\n",cur_system_time, first_time_record);	
        }
    ...
    last_checkout_vpts=(u32)get_last_checkout_pts(PTS_TYPE_VIDEO);
    last_checkout_apts = (u32)get_last_checkout_pts(PTS_TYPE_AUDIO);
    ...
    if ((play_mode != PLAY_MODE_FORCE_SLOW) && (play_mode != PLAY_MODE_FORCE_SPEED) && (tsync_pcr_usepcr == 1) && tsync_pcr_stream_delta != 0) {
       ...
       // u64 cur_pcr=(u64)timestamp_pcrscr_get();
        ...
        int pcr_audio_delay = last_checkin_apts - last_checkout_apts;
        int pcr_video_delay = last_checkin_vpts - last_checkout_vpts;

		int min_cache_time = pcr_audio_delay > pcr_video_delay ? pcr_video_delay:pcr_audio_delay;

		int  fast_flag = (min_cache_time > tsync_pcr_max_cache_time);
		int  slow_flag = (min_cache_time < tsync_pcr_min_cache_time);

		int normal_flag = ((slow_flag == 0)&&(fast_flag == 0));
       
       /* printk("checkin  apts %llx checkout apts %llx checkin vpts %llx checkout vpts %llx,min_cache_time %d \n ",last_checkin_apts,\
                last_checkout_apts,last_checkin_vpts,last_checkout_vpts,min_cache_time);
*/
	if(fast_flag && (play_mode!=PLAY_MODE_SPEED) && need_recovery)
	{
		...
		amlog_level(LOG_LEVEL_INFO, "[tsync_pcr_check] tsync_pcr_max_cache_time=%d min_cache_time=%d to speed play  \n",tsync_pcr_max_cache_time,min_cache_time);
        }
		else if(slow_flag && (play_mode!=PLAY_MODE_SLOW) && need_recovery)
		{
           ...
           // amlog_level(LOG_LEVEL_INFO, "[tsync_pcr_check] cur_pcr=%lld to slow play  \n",cur_pcr);
			amlog_level(LOG_LEVEL_INFO, "[tsync_pcr_check] tsync_pcr_max_cache_time=%d min_cache_time=%d to slow play  \n",tsync_pcr_max_cache_time,min_cache_time);
        }
	    else if((normal_flag)&&(play_mode!=PLAY_MODE_NORMAL))
		{
            play_mode=PLAY_MODE_NORMAL;
            ...
            amlog_level(LOG_LEVEL_INFO, "[tsync_pcr_check] min_cache_time=%d  to normal play \n",min_cache_time);
        }
    }
...
u32  tsync_get_firstapts(void)
{
        return tsync_pcr_first_audio_frame_pts;
}


EXPORT_SYMBOL(tsync_get_firstapts);
