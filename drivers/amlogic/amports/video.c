...
extern void am_i2s_trigger_output(void);
...
amlog_mask(LOG_MASK_FRAMEINFO,
    ...
    if (first_picture) {
        if(vf->source_type == VFRAME_SOURCE_TYPE_OSD)
        {
            printk("video first picture osd  display pts %x pcr %x \n", vf->pts, timestamp_pcrscr_get());   
        }
        else
        {
            printk("video first picture video display pts %x pcr %x \n", vf->pts, timestamp_pcrscr_get());
            am_i2s_trigger_output();
        }
        frame_par_ready_to_set = 1;
