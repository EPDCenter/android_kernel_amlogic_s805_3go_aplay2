...
// code to force enable none-samesource to trigger alsa reset audio hw,only once available
static ssize_t audio_samesource_store(struct class *class,
                                      struct class_attribute *attr,
                                      const char *buf, size_t count)
{
	audio_samesource  = simple_strtoul(buf, NULL, 10);
	printk("same source set to %d \n", audio_samesource);
	return count;
...
static ssize_t audio_samesource_show(struct class *cla, struct class_attribute *attr, char *buf)
{
	char *pbuf = buf;
	int samesource = 0;
	int i2s_enable = !!(READ_MPEG_REG_BITS(AIU_MEM_I2S_CONTROL, 1, 2));
	int iec958_enable = !!(READ_MPEG_REG_BITS(AIU_MEM_IEC958_CONTROL, 1, 2));
	samesource = (READ_MPEG_REG(AIU_MEM_IEC958_START_PTR) == READ_MPEG_REG(AIU_MEM_I2S_START_PTR));
