...
{
	int lnb_gpio = 22;//GPIOH_8 ,value can be found in uboot partion(arch/arm/include/asm/arch-m8b/gpio.h)
	AVL_DVBSx_ErrorCode r=AVL_DVBSx_EC_OK;	
	AVL_uchar nValue = 1;
	pr_dbg("\n------------->SEC_VOLTAGE_OFF = %d\n",SEC_VOLTAGE_OFF);
	pr_dbg("\n------------->SEC_VOLTAGE_13 = %d\n",SEC_VOLTAGE_13);
	...
	if(voltage == SEC_VOLTAGE_OFF){
		...
		{
#if LINUX_VERSION_CODE>= KERNEL_VERSION(3,10,0)	
			amlogic_gpio_request(lnb_gpio,device_name);
			amlogic_gpio_direction_output(lnb_gpio, 0, device_name);
#else
			gpio_out(lnb_gpio, 0);
#endif
		}
...
#if LINUX_VERSION_CODE>= KERNEL_VERSION(3,10,0)
		amlogic_gpio_request(lnb_gpio,device_name);
		amlogic_gpio_direction_output(lnb_gpio, 1, device_name);
#else
		gpio_out(lnb_gpio, 1);
