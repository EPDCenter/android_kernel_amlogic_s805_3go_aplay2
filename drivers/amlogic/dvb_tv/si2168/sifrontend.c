...
#include "include/sifrontend.h"
//#include "../../../../../common/drivers/amlogic/dvb_tv/aml_fe.h"

static int debug_si2168 = 0;

#define pr_dbg(fmt, args...)\
	do{\
		if(debug_si2168)\
			printk("Si2168 debug: " fmt, ## args);\
	}while(0)
#define pr_error(fmt, args...) printk("Si2168 error: " fmt, ## args)
...
static int si2168_get_property(struct dvb_frontend *fe, struct dtv_property *p)
{
	...
	Si2168_L1_DVBT2_STATUS (api, 1/*Si2168_DVBT_STATUS_CMD_INTACK_CLEAR*/);
  ...
  //  lock = SiLabs_API_Channel_Seek_Next(&front_end, &standard, &freq,  &bandwidth_Hz, &stream, &symbol_rate_bps, &constellation, &polarization, &band, &num_plp);
	switch (p->cmd) {
		case DTV_DVBT2_PLP_ID:
			{
				if(p->u.buffer.reserved1[1]!=UINT_MAX){
					pr_dbg("get plp_id\n");
				...
				if (num_plp < 1) {
					...
					/*
					   p->u.data==UINT_MAX for case DTV_DVBT2_DATA_PLPS
					   to get the data plps
					*/
					uint8_t plpids[256];
					uint8_t plpnum = 0;
					pr_dbg("get plps\n");
