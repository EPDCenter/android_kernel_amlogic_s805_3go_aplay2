...
#include "../include/SiLabs_API_L3_Wrapper.h"
...
int   SiLabs_API_Demod_status             (SILABS_FE_Context* front_end, CUSTOM_Status_Struct *status)
{
  ...
  if (front_end->chip ==   2168 ) {
    ...
    //    status->RSSI               = front_end->Si2168_FE->tuner_ter->rssi;
      ...
      case SILABS_DVB_T2 : {
        if (Si2168_L1_DVBT2_STATUS   (front_end->Si2168_FE->demod, Si2168_DVBT2_STATUS_CMD_INTACK_OK)                       != NO_Si2168_ERROR) 
	 {
		SiTRACE("\n---------------->Si2168_L1_DVBT2_STATUS  != NO_Si2168_ERROR\n");
		return 0;
	 }
  ...
  SiTRACE("SiLabs_API_Text_status in %s\n", Silabs_Standard_Text(status->standard));
    ...
    switch (status->standard)
    {
        ...
        case SILABS_DVB_T : {
          ...
          sprintf(formatted_status, "%s %d MHz \n"       , formatted_status, (int)(status->bandwidth_Hz/1000000));
          break;
        }
        case SILABS_DVB_C : {
          sprintf(formatted_status, "%s characteristics     %s ", formatted_status, Silabs_Constel_Text(status->constellation));
          sprintf(formatted_status, "%s %d Mbps \n"          , formatted_status, (int)(status->symbol_rate/1000000));
          break;
        }
        case SILABS_DVB_C2 : {
        ...
        case SILABS_DVB_S : {
          sprintf(formatted_status, "%s characteristics     %s ", formatted_status, Silabs_Constel_Text(status->constellation));
          sprintf(formatted_status, "%s %d Mbps "           , formatted_status, (int)(status->symbol_rate/1000000));
...
//          sprintf(formatted_status, "%s %1.3f Mbps "           , formatted_status, status->symbol_rate/1000000.0);
        ...
        case SILABS_DVB_S2: {
          ...
          sprintf(formatted_status, "%s fer              %d\n", formatted_status, (int)status->fer);
          }
          break;
        }
        case SILABS_DVB_T2: {
          ...
          /*sprintf(formatted_status, "%s %s "                 , formatted_status, Silabs_Coderate_Text(status->code_rate));*/
          sprintf(formatted_status, "%s %s "                   , formatted_status, Silabs_GI_Text(status->guard_interval));
          sprintf(formatted_status, "%s %d MHz "            , formatted_status, (int)(status->bandwidth_Hz/1000000));
    ...
    sprintf(formatted_status, "%s ber              %d\n"   , formatted_status, (int)(status->ber));
    }
    if (status->per     == -1) {
    sprintf(formatted_status, "%s per              --------\n", formatted_status);
    } else {
    sprintf(formatted_status, "%s per              %d\n"   , formatted_status, (int)(status->per));
    }
    sprintf(formatted_status, "%s c_n              %d\n"   , formatted_status, (int)(status->c_n));
    if (status->uncorrs == -1) {
    ...
    sprintf(formatted_status, "%s RSSI             %d\n"     , formatted_status, (int)(status->RSSI));
    sprintf(formatted_status, "%s SSI              %d\n"     , formatted_status, (int)(status->SSI));
...
#ifdef    Si2168_COMPATIBLE
  if (chip == 2168) {
    front_end->Si2168_FE = (struct Si2168_L2_Context  *)&(front_end->Si2168_FE_Obj);
...
{
  front_end = front_end; /* To avoid compiler warning */
  return 0;//L0_ReadString (WrapperI2C, readString, pbtDataBuffer);
}
/************************************************************************************************************************
...
{
  front_end = front_end; /* To avoid compiler warning */
  return 0;//L0_WriteString(WrapperI2C, writeString);
}
/************************************************************************************************************************
